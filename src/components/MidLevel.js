        ...
        if (this.state.active === true) {
			...
			<div className='playZone'>
				<table className='tablePlayZone'>
				 <tbody>
					<tr>
						{this.state.playZone2.map((element,index) => {
							return(
							<td key={`0${index}`} onClick={this.state.playZone2[0][index] === 1 ? this.startGame : null}><GameBlock isActive={this.state.playZone2[0][index]}
					...
					<tr>
						{this.state.playZone2.map((element,index) => {
							return(
							<td key={`1${index}`} onClick={this.state.playZone2[1][index] === 1 ? this.startGame : null}><GameBlock isActive={this.state.playZone2[1][index]}
																																	playing={this.state.active}
																																	difficulty={this.props.difficulty}/></td>
							)
						})}
					...
					<tr>
						{this.state.playZone2.map((element,index) => {
							return(
							<td key={`2${index}`} onClick={this.state.playZone2[2][index] === 1 ? this.startGame : null}><GameBlock isActive={this.state.playZone2[2][index]}
																																	playing={this.state.active}
																																	difficulty={this.props.difficulty}/></td>
							)
						})}
					...
					<tr>
						{this.state.playZone2.map((element,index) => {
							return(
							<td key={`3${index}`} onClick={this.state.playZone2[3][index] === 1 ? this.startGame : null}><GameBlock isActive={this.state.playZone2[3][index]}
																																	playing={this.state.active}
																																	difficulty={this.props.difficulty}/></td>
							)
						})}
